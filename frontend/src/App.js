...
const App = (props) => {
  ...
  useEffect(() => {
    if (props.socket) {
      ...
      props.socket.on('accepted_game_request', ({ username, requests, playing_now, game, coins }) => {
      ...
      props.socket.on('accepted_friend_request', ({ username, requests, friends }) => {
  ...
  return (
    <BrowserRouter>
      ...
      <Switch>
        ...
        <Route path='/selectgame' component={!props.token ? Account : GameButtons} />
